...
public class RoutineTwoController {
    ...
    /**
     * Simple execution function that executes routine 3 of the optimizer.
     * This routine handles the minimum ratio required for work:health is above the required amount. For every 4 hours of work there must be 30 minutes of health based activity.
     */
    public void runRoutine(CalendarSource mainSource) {
        ...
        List<Goal> healthGoalList = getHealthGoalList();
        List<Goal> parsedHealthList = parseGoalList(healthGoalList);



        Calendar goalCalendar = getGoalCalendar(mainSource.getCalendars());
        Calendar healthCalendar = getHealthCalendar(mainSource.getCalendars());
        ...
        for (Goal goal : parsedHealthList) {
            List<Event> eventList = eventDAO.getAllEvents();
            System.out.println("eventList size: " + eventList.size());
            System.out.println("Starting goal " + goal.getTitle());
            // need to change only alocates if goal calendar is 
            allocateGoal(goal, eventList, healthCalendar);
        }

    }

    /**
     * Takes in a list of calendars and returns the calendar with the name Health
     *
     * @param calendars List of calendars in CalendarSource
     * @return Calendar object representing Goals calendar
     */
    private Calendar getHealthCalendar(List<Calendar> calendars) {
        for (Calendar calendar : calendars) {
            if (Objects.equals(calendar.getName(), "Health"))//** changed to "Health"
             {
                return calendar;
            }
        }
    ...
    private Calendar getGoalCalendar(List<Calendar> calendars) {
        for (Calendar calendar : calendars) {
            if (Objects.equals(calendar.getName(), "Goals"))//** changed to Health
             {
                return calendar;
            }
        }
    ...
    private List<Goal> getHealthGoalList() {
        List<Goal> healthGoalList = new ArrayList<>();
        healthGoalList.add(goalThree);
    ...
    private List<Goal> parseGoalList(List<Goal> goalList) {
        ...
        for (Goal goal : goalList) {
            ...
            //***add only if goal type is belonging to the health calendar
